...
#     t2 = threading.Thread(target=run,args=('t2',))     # args：参数
#     t1.start()
#     t2.start()
...
#         self.n = n                        # super() 函数是用于调用父类(超类)的一个方法。
#                                           # super是用来解决多重继承问题的，直接用类名调用
#                                           # 父类方法在使用单继承的时候没问题，
#                                           # 但是如果使用多继承，会涉及到查找顺序（MRO）、
#                                           # 重复调用（钻石继承）等种种问题。
#
#     def run(self):
#         print('task',self.n)
